...
function addSpan(cov, S, row, xa, xb, w) {
  xa = clamp(xa, 0, S);
  xb = clamp(xb, 0, S);
  if (xb <= xa) return;
...
function rasterizeEvenOdd(edges, S) {
  ...
  for (let row = 0; row < S; row++) {
    for (let k = 0; k < SUB; k++) {
      ...
      xs.sort((a, b) => a - b);
      for (let i = 0; i + 1 < xs.length; i += 2) addSpan(cov, S, row, xs[i], xs[i + 1], w);
    }
  }
  return cov;
}
...
// Render the yellow M2 badge on a transparent background at the given size.
// The 1024 viewBox is fit into ~90% of the canvas, centered.
const subpaths = parsePath(M2_PATH);
function renderRGBA(S) {
  const draw = S * 0.9;
  const cov = rasterizeEvenOdd(buildEdges(subpaths, draw / 1024, (S - draw) / 2, (S - draw) / 2), S);
...
// --- Minimal ICO encoder (wraps PNG images; Vista+ supports PNG-in-ICO) ---
...
// 512px PNG -> window icon (src/assets) + electron-builder PNG fallback (build/).
const png512 = encodePng(renderRGBA(512), 512, 512);
writeFile(path.join('src', 'assets', 'icon.png'), png512);
writeFile(path.join('build', 'icon.png'), png512);

// Multi-size ICO -> NSIS installer + app icon (electron-builder reads build/icon.ico).
const ICO_SIZES = [16, 24, 32, 48, 64, 128, 256];
const ico = encodeIco(ICO_SIZES.map((size) => ({ size, png: encodePng(renderRGBA(size), size, size) })));
writeFile(path.join('build', 'icon.ico'), ico);
